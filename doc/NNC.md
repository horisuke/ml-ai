...
* まず左側のGlobal Config画面にて、以下の通り設定する。
    * Max Epoch：20
    * Batch Size：100
* 次にネットワーク構造の最適化機能を有効にするため"Structure Search"の"Enable"にチェックを入れる。
* これにより、学習を繰り返す過程でネットワーク構造が変化し、より精度が高く演算量の少ないネットワーク構造を探すことができる。
* また、"Enable"にすることでネットワーク構造の最適化を行なう手法を以下から選択することが可能となる。ここでは"Random"を選択する。
    * Random：
        * ランダムに試行錯誤を繰り返し、最適なネットワークを探す。最適化に時間がかかる。
    * Network Feature + Gaussian Process：
        * ネットワーク特徴量とGaussian Processを用い、Bayesian Optimizationによって、最適なネットワークを探す。"Random"を選択するよりも短い時間で最適化できる。
* "Optimize for"では何を対象に最適化するかを以下から選択することができる。
    * Error：誤差の小ささを重視して最適なネットワークを探す(演算量の大きさは考慮しない)。最適化手法として、"Network Feature + Gaussian Process"を選択した場合のみ選択できる。
    * Error and Calculation：誤差及び演算量の小ささを重視して最適なネットワークを探す。
* "Search Range"ではValidation(精度)、Multiply Add(乗加算回数)の探索範囲を最小値/最大値で指定することができる。ここでは設定しないものとする。
* "Max Attempts"では、最大実行時間、最大試行回数が設定できる。どちらかの設定値を満たすまでネットワーク構造最適化のための学習が行われる。
* ここではそれぞれ以下の通り設定する。
    * 最大実行時間：1 hours
    * 最大試行回数：10 times　※実際は自分で構築したネットワーク構造における学習も含めると+1回多く実行される。
* 次に左側のOptimizer画面にてUpdaterを以下の通り変更する。それ以外の設定はそのままとする。
    * adam → sgdに変更
    * LearningRate：0.01 → 0.5
* ここまでの設定で学習を行なう。EDIT画面を選択し、右上のRunボタンを選択し、学習を開始する。
* 学習を開始するとTRAININGタブに遷移し、学習が始まる。
    * 左側のJob Historyに学習ごとの結果のサマリが表示される。
    * サマリを選択すると、画面中央にはLearning Curve(Epochを横軸、縦軸にCost、Training Error、Validation Errorの値を取ったグラフ)が表示され、画面中央下部には実行時のログが表示される。
    * 画面右側には、Overviewとして、学習に使用したモデルの構造が表示される。また同様にそのネットワークの統計データも表示される。
* 各学習は前回の学習からネットワーク構造がなんらか変化しており、新しいネットワーク構造において同様に学習を行なう。
* Job Historyは"20190620_081629_x"という形になり、xは学習を停止する、もしくはMax Attemptsの設定条件を満たして学習が停止されるまでインクリメントされて記録される。
* 上記の設定の場合、"最大試行回数"の設定が先に設定値に達するので、自分で構築したネットワーク構造における学習履歴"20190620_081629"とネットワーク構造最適化のために行われた学習"20190620_081629_1"～"20190620_081629_10"の合計11回分が学習履歴として残る。
* 次に行われた学習履歴から1つを選び、ネットワークの評価を行なう。
* ここでは、"20190620_081629_1"で生成されているネットワークを選択する。
* 画面右側のRunボタンを実行することで、EVALUATIONタブに遷移し、ネットワークの評価が実行が開始される。
* しばらく待つと、Output ResultやConfusion Matrixが確認できるようになる。
* ★★～P.117★★
