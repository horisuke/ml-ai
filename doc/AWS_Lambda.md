...
* 実行環境はAWSによって実行のたびに用意されるため、サーバシステムの保守・運用は不要。
...
* Lambdaの課金単位は実行時間。実行されていないときは全く費用がかからない。
...
* Lambda関数はAWS上のサービスと連携して実行できる。実行のトリガーになるものをイベントソースと呼ぶ。
...
* そのため、Lambdaプログラミングのほとんどは様々なAWSサービスを糊付けするために使われる。そのため、どのようなAWSサービスがあり、どのように利用するものかという知識も不可欠になってくる。
...
* Lambda関数のサンプル例として、「x」、「y」を引数として渡すと、その値をログに出力し、「x÷y」の計算結果を戻り値として返すものとする。
...
* 「x÷y」を計算するためにint関数を使って文字列から整数に変換し、それぞれx, yに格納する。
* aa
