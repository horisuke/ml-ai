...
* 以下の手順でLambdaコンテナをサブネットに配置する。
    ...
    * "設定"タブの"ネットワーク"セクションのプルダウンで"非VPC"から作成済みのVPCを選択する。
...
* 以下のような呼び出し方法がある。LambdaコンソールからのLambda関数実行は同期呼び出しとなっているが、多くの場合、非同期呼び出しが使われる。
    ...
    * 非同期呼び出し
        ...
        * Lambda関数の戻り値は破棄される。
* 非同期呼び出しでは、イベントがキューに送信される。
* よって、呼び出しから実際の実行までに少しタイムラグが生じることがある。
* またキューから取り出されるイベントはひとつずつではなく、多くの場合、同時に複数取り出され、Lambda関数は並列に実行される。
* イベントをキューから取り出し、Lambda関数を実行し、その実行がエラーになった場合は、CloudWatchLogsに記録される。
* そのしばらく後に最大2回の試行が行われる。つまりLambda関数は最大3回呼び出される可能性がある。
* 3回とも失敗した場合、デフォルトではそのイベントは消失する。
...
* イベントソースには以下があり、その種類によって「非同期」、「同期」、「ストリーム」のどのタイプで動作するかが決まる。
    ...
    |CloudWatchEvents|非同期|CloudWatchイベント(EC2インスタンスの状態やAutoScalingサービスの状態が変化したときなど)が変化したとき、スケジュールされた時間になったときなど|
...
* 上表のイベントソース以外のサービスでも間接的にイベントソースとして利用できるものがある。
* そのためにS3やSNSがよく利用される。
    * S3の使用例
        * a
    * SNSの使用例
