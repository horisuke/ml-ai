...
* 自動着色
    * 深層学習による自動着色は、近年盛り上がりを見せており、特にイラストの自動着色がイラストレーター・デザイナーの業界で大きなブレイクスルーとなっている。
    * イラストへの着色を自動で行うことで時間を短縮し、作業効率の向上を図ろうとしている。
    * 既にPFNのPaint Chainer、アイビスモバイルのアイビスペイントなどの自動着色を行うことができるアプリケーションが開発されている。
    * 以下ではイラストへの自動着色より難しいとされる写真鵜を対象に自動着色を行う。
    * 自動着色を行なうための工夫
        * 自動着色を行うためには以下のような独自の前処理・後処理・モデルの構築が必要となる。
            * 画像の表現方法変更
                * これまでは、RGB形式の画像を扱ってきたが、自動着色を行なうモデルでは、前処理として画像の表現方法をRGBからLABに変更し、Lの値を入力してABの値を出力するようなモデルを構築する。
            * CAEのモデルからいくつか構成する層を変更
                * Encoderのプーリング層(マックスプーリング)をスライドを2とした畳み込み層に変更する。
                * この畳み込み層でデータの縮小と畳み込み処理を行なう。
                * Decoderのアップサンプリング層を転置畳み込み層(transposedCOnv2D)に変更する。
...
* 超解像
    * CAEは自動着色以外にも様々なタスクに活用でき、超解像もその1つ。
    * 超解像とは解像度の低い画像や動画を受け取り、解像度の高い画像や動画を生成する技術のこと。
    * 超解像にはいくつか種類があるが、ここでは、1枚の低解像度画像から1枚の高解像度画像を生成する「単一画像超解像」というタスクを扱う。
    * SRCNN(Super-Resolution Convolutional Neural Network)
        * SRCNNは非常にシンプルだが、従来の手法を凌駕する精度が得られて手法として知られている。
        * SRCNNは出力層も含めて畳み込み層を3層重ねただけのシンプルなモデルである。
        * SRCNNの論文では、超解像に関しては、必ずしも層を増やせばよいわけではない、という結論に至っている。
        * またシンプルなため、高速な処理が可能で、動画のオンライン処理などにも向いている技術である。
    * データの前処理
        * SRCNNの入力は低解像度の画像で、出力は高解像度の画像となるため、学習には低解像度の画像と高解像度の画像のペアが必要となる。
        * 低解像度の画像は高解像度の画像から創り出すことができるため、画像を一旦小さな画像にリサイズして、もう1度サイズを戻すことで、疑似的に低解像度の画像データを生成する。
        * a
